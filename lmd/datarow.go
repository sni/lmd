...
func (d *DataRow) setID() (err error) {
	...
	for i, k := range d.DataStore.Table.PrimaryKey {
		...
		key.WriteString(fmt.Sprintf("%v", d.GetValueByName(k)))
	}
	d.ID = key.String()
	if d.ID == "" || d.ID == ";" {
...
func (d *DataRow) setReferences(store *DataStore) (err error) {
	for i := range store.Table.RefTables {
		...
		for i := range ref.Columns {
			...
			key.WriteString(fmt.Sprintf("%v", d.GetValueByColumn(ref.Columns[i])))
		}
		refValue := key.String()
		if refValue == "" {
...
func (d *DataRow) GetString(col *Column) *string {
	...
	return interface2string(d.GetValueByColumn(col))
}
...
func (d *DataRow) GetStringList(col *Column) *[]string {
	...
	return interface2stringlist(d.GetValueByColumn(col))
}
...
func (d *DataRow) GetFloat(col *Column) float64 {
	...
	return interface2float64(d.GetValueByColumn(col))
}
...
func (d *DataRow) GetInt(col *Column) int64 {
	...
	return interface2int64(d.GetValueByColumn(col))
}
...
func (d *DataRow) GetIntList(col *Column) []int64 {
	...
	return interface2int64list(d.GetValueByColumn(col))
}
...
func (d *DataRow) GetHashMap(col *Column) map[string]string {
	...
	return interface2hashmap(d.GetValueByColumn(col))
}

// GetValueByName returns the raw value for given column name
func (d *DataRow) GetValueByName(name string) interface{} {
	col := d.DataStore.Table.ColumnsIndex[name]
	if col == nil {
		log.Panicf("no column name %s in table %s", name, d.DataStore.Table.Name)
	}
	return d.GetValueByColumn(col)
}
...
func (d *DataRow) getStatsKey(res *Response) string {
	...
	for i := range res.Request.RequestColumns {
		...
		value := d.GetValueByColumn(col.Column)
...
func (d *DataRow) CheckChangedValues(data *[]interface{}, columns *ColumnList) bool {
	for j := range *columns {
		if (*data)[j] == d.GetValueByColumn((*columns)[j]) {
			return true
		}
	}
	return false

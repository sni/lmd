...
func (req *Request) ParseRequestHeaderLine(line []byte, options ParseOptions) (err error) {
	...
	case "columns":
		...
		return nil
	case "responseheader":
		return parseReqResponseHeader(&req.ResponseFixed16, args)
	case "outputformat":
...
func parseReqResponseHeader(field *bool, value []byte) (err error) {
...
func (req *Request) parseResult(ctx context.Context, resBytes []byte) (ResultSet, *ResultMetaData, error) {
	...
	if req.OutputFormat == OutputFormatWrappedJSON {
		res, err2 := parseWrappedJSONMeta(resBytes, meta, req)
		if err2 != nil {

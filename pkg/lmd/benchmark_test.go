...
func BenchmarkPeerUpdateServiceInsert(b *testing.B) {
	...
	_, lastCheckIdx := table.getUpdateColumn("last_check", 2)

	b.StartTimer()
	for b.Loop() {
		res[0][lastCheckIdx] = b.N
		res, iErr := data.insertDeltaDataResult(2, res, meta, table)

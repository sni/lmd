...
func logHeapTrace(lmd *Daemon) {
	...
	writeHeapTrace(lmd.flags.flagMemProfile, log.Warnf)
}

func writeHeapTrace(fileName string, logger func(string, ...any)) {
	file, err := os.Create(fileName)
	if err != nil {
		logger("could not create memory profile: %s", err.Error())
	}
	defer file.Close()
	runtime.GC()
	if err := pprof.WriteHeapProfile(file); err != nil {
		logger("could not write memory profile: %s", err.Error())
	}
	logger("memory profile written to: %s", fileName)
}
